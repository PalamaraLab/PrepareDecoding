...
Matrix<double> raw_sfs(const PdModel& model, int n, double t1, double t2, bool below_only) {
  ParameterVector pv = make_params_from_model(model);
  bool bo = below_only;
  Matrix<adouble> sfs = sfs_cython(n, pv, t1, t2, bo);
...
ParameterVector make_params_from_model(const PdModel& model) {
  return make_params(model.getA(), model.getS());
}
